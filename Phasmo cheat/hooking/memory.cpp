...
uint8_t* signature::resolveRelativeAddr(uint32_t nRVA, uint32_t nRIP)
{
	if (!this->imported)
		*this = this->import();

	uint32_t _nRVA = *reinterpret_cast<uint32_t*>(this->pointer + nRVA);
	uint64_t _nRIP = this->pointer + nRIP;

	return reinterpret_cast<uint8_t*>(_nRVA + _nRIP);
}
