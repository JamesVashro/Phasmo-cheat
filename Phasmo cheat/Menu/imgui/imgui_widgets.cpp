...
bool ImGui::ButtonEx(const char* label, const float round, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(bb.Min, bb.Max, col, true, round);
...
bool ImGui::HotKeyEx(const char* label, bool* v, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGui::SameLine();
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
        pos.y += window->DC.CurrLineTextBaseOffset - style.FramePadding.y;
    ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    pos.x += ImGui::GetContentRegionAvailWidth() - size.x;
    ...
    // Render
    style.Colors[ImGuiCol_Button] = ImColor(115, 115, 115, 20);
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, 2);
    style.Colors[ImGuiCol_Button] = ImColor(80, 80, 80);
...
bool ImGui::Button(const char* label, const float round, const ImVec2& size_arg)
{
    return ButtonEx(label, round, size_arg, ImGuiButtonFlags_None);
}

bool ImGui::HotKey(const char* label, bool* v, const ImVec2& size_arg)
{
    return HotKeyEx(label, v, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::SmallButton(const char* label)
{
    ...
    bool pressed = ButtonEx(label, g.Style.FrameRounding, ImVec2(0, 0), ImGuiButtonFlags_AlignTextBaseLine);
...
void ImGui::ImageRounded(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    if (border_col.w > 0.0f)
    {
        window->DrawList->AddImageRounded(user_texture_id, bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), uv0, uv1, GetColorU32(tint_col), 75);
        window->DrawList->AddCircle(ImVec2(bb.Min.x + (size.x / 2), bb.Min.y + (size.y / 2)), size.x / 2, GetColorU32(border_col));
    }
    else
    {
        window->DrawList->AddImageRounded(user_texture_id, bb.Min, bb.Max, uv0, uv1, GetColorU32(tint_col), 75);
    }
}
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (p_step != NULL)
    {
        ...
        if (ButtonEx("-", style.FrameRounding, ImVec2(button_size, button_size), button_flags))
        {
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_inputs || allow_opt_datatype)
        Separator();
    if (Button("Copy as..", g.Style.FrameRounding, ImVec2(-1, 0)))
        OpenPopup("Copy");
    if (BeginPopup("Copy"))
    {
        int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]), cb = IM_F32_TO_INT8_SAT(col[2]), ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255 : IM_F32_TO_INT8_SAT(col[3]);
...
bool ImGui::SelectableCustom(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (hovered)
    {
        ImRect grow_bb(bb.Min.x, bb.Min.y, bb.Min.x + (width * t), bb.Max.y);
        const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
        RenderFrame(grow_bb.Min, grow_bb.Max, col, false, 5.f);
    }
...
bool ImGui::SelectableCustom(const char* label, bool* p_selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    if (SelectableCustom(label, *p_selected, flags, size_arg))
    {
        *p_selected = !*p_selected;
        return true;
    }
